...
symphony_version=0.48.20                 #Symphony Helm Chart version
agent_version=0.48.4                     #Symphony Agent version
kanportal_version=0.48.20-amd64          #Kanportal Helm Chart version
kanai_version=0.48.20                    #KanAI container version
...
while [ $current_step -lt 11 ]; do
    case $current_step in
        ...
        5 ) 
            while true; do
                ...
                case $REPLY in
                    ...
                    ;;
                    * )
                        display_error_message "please enter y/n"
                esac
            done
            current_step=`expr $current_step + 1` 
        ;;
        ...
        7 )
            while true; do
                echo "How would you like the KAN portal service to be exposed:"
                echo -e "  \033[0;33m1)\033[0m Install and use a Nginx Ingress Controller"
                echo -e "  \033[0;33m2)\033[0m Use a LoadBalancer service"
                echo -e "  \033[0;33m3)\033[0m Use an existing Traefik Ingress Controller"
                read -p "Your choice:" -r; echo
                case $REPLY in
                    1 )
                        ingress="nginx"
        ...
        10 )
            ...
            echo -e "  \033[0;94mSymphony agent image:\033[0m\t\t$agent_image"
            echo -e "  \033[0;94mKAN portal ingress:\033[0m\t\t$ingress"
            ...
            case $REPLY in
                [Yy]* )
                    ...
                    if [ $create_sp_selection == "1" ]; then
                        ...
                        while true; do
                            app_info=$(az ad sp create-for-rbac --name $selected_sp_name)
                        ...
                        echo "Please note down your service principal credentials. This will be displayed only once! Press enter to continue."
                        echo "Your service principal credentials:"
                        echo $app_info
                        echo "Press enter to continue..."   
                        read -p ""
                        sp_password=$(echo $app_info | jq -r ".password")
                        sp_tenant=$(echo $app_info | jq -r ".tenant")
                    ...
                    if [ $ingress == "nginx" ]; then
                        echo -e "  \033[0;94mInstalling ingress\033[0m"
                    ...
                    if [ $ingress == "LoadBalancer" ]; then
                        values="$values --set service.type=LoadBalancer"
                    else
                        values="$values --set service.type=ClusterIP"
                    fi
                ...
                ;;
                * )
                    current_step=`expr $current_step - 5`
            esac
        ;;
        * )
...
portalIp=""
start_progress_animation "checking KAN portal address..."
                
if [ $ingress == "LoadBalancer" ]; then
    # wait for ingress service ready
    while true; do
        portalIp=$(kubectl get svc -A | grep kanportal | grep LoadBalancer | awk {'print $5'})
...
echo -e "  \033[0;94mThe platform will be ready in few minutes at http://$portalIp\033[0m"
