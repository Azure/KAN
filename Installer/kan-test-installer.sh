...
while [ $current_step -lt 8 ]; do
    ...
    5 )
        while true; do
            ...
            case $REPLY in
                1 )
                    agent_version=kantest.azurecr.io/kan-agent:$kan_version      
                ;;
                2 )
    ...
    7 )
        ...
        case $REPLY in
            [Yy]* )
                ...
                helm upgrade -n default --install kanportal oci://kantest.azurecr.io/helm/kanportal --version $kanportal_version $values --set image.image=kantest.azurecr.io/kanportal --set kanAgentImage=$agent_version
