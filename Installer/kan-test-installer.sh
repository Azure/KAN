    ...
    5 )
        ...
        case $REPLY in 
            [Yy]* )            
                ...
                if [ $create_custom_vision_selection == 3 ]; then
                    helm upgrade -n default --install kan oci://p4etest.azurecr.io/helm/kan --version $kan_version --wait
                else 
                    helm upgrade -n default --install kan oci://p4etest.azurecr.io/helm/kan --set CUSTOM_VISION_KEY=$(az cognitiveservices account keys list -n $selected_custom_vision_name -g $selected_custom_vision_rg | jq -r ".key1") --version $kan_version --wait
                fi
