    ...
    5 )
        ...
        case $REPLY in 
		    ...
		    sleep 120
                ...
                helm upgrade --install symphony oci://possprod.azurecr.io/helm/symphony --set CUSTOM_VISION_KEY=$(az cognitiveservices account keys list -n $selected_custom_vision_name -g $selected_custom_vision_rg | jq -r ".key1") --version 0.39.7 --wait
                if [ $? != "0" ];  then
                ...
                helm upgrade --install voe oci://possprod.azurecr.io/helm/voe --version 0.39.0-main-9171c1e-amd64 \
