...
4. For each line that references the LoadBalancer IP address: 
  a. Select the inbound security rule. 
  b. Change the source IP address range to the network you want to allow access to the portal. For example, your home or corporate IP address space in classless inter-   domain routing (CIDR) notation. 
...
2. Use Kubernetes port-forwarding to access the portal on the spot locally using the following command:  
  ...
  '''
  This allows you to view the portal using: localhost:<you-desired-port> 
...
3. Authentication and managing user accounts 
  a. [How to manage user accounts in Azure API Management](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-create-or-invite-developers) 
  b. [Authorize access to API Management developer portal by using Azure AD - Azure API Management](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-aad?WT.mc_id=Portal-fx) 
