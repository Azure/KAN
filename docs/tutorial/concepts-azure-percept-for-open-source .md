...
This article covers the following six key concepts to provide you with a deep understanding of Azure Percept for Open-Source Project portal concepts:
...
Azure Percept for Open-Source Project and its Edge AI solutions are supported on many different devices and accelerators, for example, NVIDIA Orin AGX/NX, Xavier AGX/NX, Azure Stack HCI, and Azure stack edge. POSS also supports K8s and IoT Edge devices, including AKS, AKS-HCI, AKS-IoT and Arc-enabled K8s clusters.
...
You defined the target (compute device) in the first step. This is your Edge virtual machine (VM). Targets can also be on other K8s environments that you want to run on IoT edge environments. You can expand your targets to other environments, including K8s AKS HCI, K3s AKS-IoT, and Iot Edge devices such as DDK, EFLOW, and Percept VM.
