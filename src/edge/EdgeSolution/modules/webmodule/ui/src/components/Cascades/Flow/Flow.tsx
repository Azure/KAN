...
const getNodeParams = (modelId: string, modelList: any[]) => {
...
const getSourceMetadata = (
  ...
  const matchModels = elements
    ...
    .map((model: TrainingProject) => model.outputs.find((output) => output.route === 'bounding_box'))
    .filter((model) => !!model);

  if (matchModels.length === 0) return [];
  return matchModels[0].route;
};
      ...
      <ReactFlowProvider>
        ...
        <div className="reactflow-wrapper" ref={reactFlowWrapper}>
          <NodePanel
            ...
            matchTargetLabels={getSourceMetadata(selectedNode, elements, modelList) as string[]}
          />
          <ReactFlow
            ...
            nodeTypes={{
              source: (node: Node) => {
                ...
                console.log('data.connectMap', data.connectMap);
