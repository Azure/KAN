...
interface Props {
  exisintNameList: string[];
}

const ComputeDeviceCreation = (props: Props) => {
  const { exisintNameList } = props;
  ...
  const onFormDateValidate = useCallback(
    (currentStep: PivotTabKey) => {
      ...
      if (exisintNameList.includes(localFormData.name)) {
        setLocalFormData((prev) => ({ ...prev, error: { ...prev.error, name: ERROR_NAME_BE_USED } }));
        return true;
      }
      if (localFormData.iothub === '') {
        setLocalFormData((prev) => ({ ...prev, error: { ...prev.error, iotHub: ERROR_BLANK_VALUE } }));
        return true;
      }
      if (localFormData.iotedge_device === '') {
        setLocalFormData((prev) => ({
          ...prev,
          error: { ...prev.error, iotedge_device: ERROR_BLANK_VALUE },
        }));
        return true;
      }
