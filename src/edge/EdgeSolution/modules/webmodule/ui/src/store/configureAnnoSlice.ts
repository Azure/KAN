...
// import { Line, LineType, isLine } from './shared/SingleLine';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    onCreateConfigureAnnoClick: (state, action: PayloadAction<{ shape: Shape; purpose: Purpose }>) => {
...
export const { updateVideoAnno, onCreatingPoint, removeVideoAnno, onCreateConfigureAnnoClick, finishLabel } =
  slice.actions;

export const { selectAll: selectAllConfigureAnnos } = entityAdapter.getSelectors<State>(
  (state) => state.configureAnno,
);

export const configureAnnosSelectorFactory = (cameraId: number) =>
  createSelector(selectAllConfigureAnnos, (annos) => annos.filter((e) => e.camera === cameraId));
