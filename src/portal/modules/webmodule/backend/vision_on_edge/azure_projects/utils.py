...
def create_cv_project_helper(name: str, tags=None, project_type: str = None, classification_type: str = None):
    ...
    project_obj = Project.objects.create(name=name, setting=setting_obj, is_demo=False, project_type=project_type, category="customvision",
                                         classification_type=classification_type, is_cascade=True, type="customvision_model", node_type="model", inputs=inputs)
...
def pull_cv_project_helper(customvision_project_id: str, is_partial: bool, name: str = "", symphony_id: str = ""):
    ...
    if not setting_obj.is_trainer_valid:
    ...
    try:
        ...
        logger.warning("Could not found project, not creating model objects")
        return

    # Invalid CustomVision Project ID handled by exception
    if not name:
        name = trainer.get_project(project_id=customvision_project_id).name
    setting_obj.domain_id = trainer.get_project(
        customvision_project_id).settings.domain_id
    project_type = trainer.get_domain(
        trainer.get_project(customvision_project_id).settings.domain_id).type
    ...
    if project_obj.project_type == "Classification":
        project_obj.classification_type = trainer.get_project(
            customvision_project_id).settings.classification_type
