...
class PdObjectNotFound(NotFound):
    status_code = 404
    default_detail = "Deployment object not found."
...
class PdRelabelWithoutProject(APIException):
    status_code = 400
    default_detail = "Deployment does not have an project."
    default_code = "pd_relabel_without_project"


class PdRelabelWithoutParts(APIException):
    status_code = 400
    default_detail = "Deployment project does not have an part."
    default_code = "pd_relabel_without_parts"
...
class DeploymentVideoError(APIException):
    status_code = 400
    default_detail = (
...
class PdDeployToInfereceError(APIException):
    status_code = 503
    default_detail = (
        "Deployment deploy failed cause Inference Module does not response."
    )
...
class PdExportInfereceReadTimeout(APIException):
    ...
    default_detail = "Please refresh."
    # default_detail = (
    #     "Deployment deploy failed cause Inference Module requests timeout."
    # )
