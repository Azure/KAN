...
const sendTrainInfoToAppInsight = async (selectedParts: ProjectData['parts']): Promise<void> => {
  const { data: images } = await rootRequest.get('/api/images/');
...
export const ConfigTaskPanel: React.FC<ConfigTaskPanelProps> = ({
  ...
  const partOptionsSelector = useMemo(
    () => partOptionsSelectorFactory(projectData.trainingProject),
    [projectData.trainingProject],
  );
  const partOptions = useSelector(partOptionsSelector);
  const selectedPartOptions = useMemo(
    () => partOptions.filter((e) => projectData.parts.includes(e.key)),
    [partOptions, projectData.parts],
  );
