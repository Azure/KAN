...
>('ComputeDevice/GetSingle', async ({ id, symphony_id }) => {
  const response = await rootRquest.get(`/api/compute_devices/get_symphony_object?symphony_id=${symphony_id}`);
...
export const updateComputeDevice = createWrappedAsync<any, UpdateComputeDevicePayload>(
  ...
  async ({ id, symphony_id, body }) => {
    const response = await rootRquest.patch(`/api/compute_devices/update_symphony_object?symphony_id=${symphony_id}`, body);
...
export const selectDeviceBySymphonyIdSelectorFactory = (id: string) =>
  createSelector(selectAllComputeDevices, (deviceList) => deviceList.find((device) => device.symphony_id === id));
