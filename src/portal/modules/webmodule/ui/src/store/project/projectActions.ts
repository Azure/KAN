...
export const thunkGetProject =
  (): ProjectThunk =>
  (dispatch): Promise<boolean> => {
    dispatch(getProjectRequest());

    const getPartDetection = rootRquest.get('/api/part_detections/');
    const getInferenceModule = rootRquest.get('/api/inference_modules/');
...
export const thunkPostProject =
  (projectData: Omit<ProjectData, 'id'>): ProjectThunk =>
  (dispatch, getState): Promise<number> => {
...
export const getConfigure = createWrappedAsync<any, number>('project/configure', async (projectId) => {
  await rootRquest.get(`/api/part_detections/${projectId}/configure`);
});
